<div class="card">
    <div class="card-body">
        <form class="form-horizontal form-material" action="{{route('updateQuoteFun')}}" method="post" id="add_contact" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="id" value="{{$records->id}}">
                <div class="col-md-12 mt-4">
                    <label for="quote" class="form-label">{{__('msg.Quote')}}</label>
                    <div class="input-group">
                        <textarea id="quote" name="quote" rows="5" cols="150" placeholder=" {{ __('msg.Enter Islamic Quote')}}">{{$records->quotes}}</textarea>
                    </div>
                ...
                <div class="col-md-12 mt-4">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <div class="custom-file">
                            <input type="file" name="image" value="{{$records->image}}" class="custom-file-input" id="inputGroupFile01">
                            <label class="custom-file-label" for="inputGroupFile01">{{$records->image ? $records->image : 'Choose file'}}</label>
                        </div>
                    </div>
                </div>
                <div class="col-md-12">
