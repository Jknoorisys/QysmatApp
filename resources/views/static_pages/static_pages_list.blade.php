    ...
    <div class="card">
       <div class="card-body">
        <div class="row">
            ...
            {{-- <div class="col-2"><a href="{{route('addPage')}}" class="btn btn-qysmat mb-2">{{ __('Add Static Page')}}</a></div> --}}
        </div>
            <div class="table-responsive">
                <table class="table table-sm table-hover">
                    <thead>
                        <tr>
                            {{-- <th class="text-center">{{ __('msg.Page Name')}}</th> --}}
                            <th class="text-center">{{ __('msg.Page Title')}}</th>
                                    ...
                                    <td class="text-center bt-switch">
                                        ...
                                        <div class="row justify-content-center">
                                            ...
                                            <div class="col-2 mt-1">
                                                <form action="{{route('changePageStatus')}}" method="post" class="text-center">
                                                    @csrf
                                                    <input type="hidden" name="id" value="{{$value->id}}">
                                                    <input type="hidden" name="status" value="{{$value->status == 'Active' ? 'Inactive' : 'Active' }}">
                                                    <button type="submit" data-status="{{$value->status == 'Active' ? 'Active' : 'Inactive'}}" data-id="{{$value->id}}" data-name="{{$value->page_name}}" class="btn block_confirm btn-sm"><input type="checkbox" id="switch" {{$value->status == 'Inactive' ? '' : 'checked'}} /><label for="switch">Toggle</label></button>
                                                </form>
                                            </div>
                                            <div class="col-2">
